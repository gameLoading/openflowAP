...
int scanSSIDs(char *ifname) {
    int if_index = if_nametoindex(ifname); // Use this wireless interface for scanning.
...
static int setTxpower_callback_trigger(struct nl_msg *msg, void *arg) {
    printf("setTxpower_callback_trigger() called.\n");
    return NL_SKIP;
}
...
int setTxPower(char *ifname, int txpower) {
	...
	if (!msg) {
        ...
        syslog(LOG_INFO, "failed to allocate netlink message\n");
        closelog();
		return 2;
	}
	...
	if (!cb || !s_cb) {
        openlog("ofproto.c", LOG_CONS, LOG_USER);
        syslog(LOG_INFO, "failed to allocate netlink callbacks\n");
        closelog();
		err = 2;
        return 2;
	}
	...
	nla_put_u32(msg, NL80211_ATTR_WIPHY_TX_POWER_LEVEL, txpower);

	nl_cb_err(cb, NL_CB_CUSTOM, error_handler, &err);
	nl_cb_set(cb, NL_CB_FINISH, NL_CB_CUSTOM, finish_handler, &err);
	nl_cb_set(cb, NL_CB_ACK, NL_CB_CUSTOM, ack_handler, &err);
    nl_cb_set(cb, NL_CB_VALID, NL_CB_CUSTOM, setTxpower_callback_trigger, &results);  // Add the callback.
    nl_cb_set(cb, NL_CB_SEQ_CHECK, NL_CB_CUSTOM, no_seq_check, NULL); 

    err = 1;
    ret = nl_send_auto(socket, msg);  // Send the message.
    openlog("ofproto.c", LOG_CONS, LOG_USER);
    syslog(LOG_INFO, "NL80211_CMD_SET_WIPHY sent %d bytes to the kernel.\n", ret);
    syslog(LOG_INFO, "Waiting for setting txpower to complete...\n");
    closelog();
    if (err < 0) {
        openlog("ofproto.c", LOG_CONS, LOG_USER);
        syslog(LOG_INFO, "WARNING: err has a value of %d.\n", err);
        closelog();
    }
    if (ret < 0) {
        openlog("ofproto.c", LOG_CONS, LOG_USER);
        syslog(LOG_INFO, "ERROR: nl_recvmsgs() returned %d (%s).\n", ret, nl_geterror(-ret));
        closelog();
        return ret;
    }
    openlog("ofproto.c", LOG_CONS, LOG_USER);
    syslog(LOG_INFO, "Setting txpower is done.\n");
    closelog();
